 ...
 * <p>Description:  This class manages a collection of Shelter objects for a county.
 * It provides methods to insert new shelters, compute total beds available,
 * check if all shelters are full, find the shelter with the most availability,
 * retrieve shelters by index, and register guests across shelters.
 * </p>
...
public class CountyShelters {
	...
	public boolean registerGuests (int numGuests)
	{
		...
		if (numGuests < this.calcTotalBedsAvailable())
		{
			...
			for (int i = 0; i < numShelters; i++)
			{
				int index = this.determineShelterMostAvailIndex();
				int availBeds = places[index].determineBedsAvailable();

				if (availBeds >= numGuests)
				{
					places[index].registerGuests(numGuests);
					return true;
				}
				else
				{
					places[index].registerGuests(availBeds);
